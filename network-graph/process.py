...
from data import get_data, get_csv_data, get_upload_data
...
def make_graph(nodes, edges, frequency_pairs):
    G = NetworkGraph()
...
def reload_graph():
    edges = get_upload_data('edges.csv')
    nodes = get_upload_data('nodes.csv')
    frequency_pairs = get_upload_data('frequencies.csv')
    make_graph(nodes, edges, frequency_pairs)